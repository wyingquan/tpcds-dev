...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('snow','chocolate','maroon'))
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2000-01-06','2000-10-04','2000-11-21')))
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and store.s_city in ('Five Forks','Greenville')
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2000-08-15' as date) 
                  and (cast('2000-08-15' as date) +  30 )
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('1998-06-25' as date) - 30 )
                    and (cast ('1998-06-25' as date) + 30 )
...
select  
 ...
 where
  c.c_current_addr_sk = ca.ca_address_sk and
  ca_county in ('Ottawa County','Quitman County','Love County','Benton County','Waller County') and
  cd_demo_sk = c.c_current_cdemo_sk and 
  exists (select *
          from store_sales,date_dim
          where c.c_customer_sk = ss_customer_sk and
...
select  i_item_id
 ...
 and d_date between cast('2000-05-23' as date) and (cast('2000-05-23' as date) +  60 )
 and i_manufact_id in (71,649,24,714)
...
and cc_county in ('Richland County','Williamson County','Ziebach County','Raleigh County',
                  'Barrow County'
)
and exists (select *
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('2001-06-27' as date)) 
 ...
 and cs_warehouse_sk    = w_warehouse_sk 
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2001-06-27' as date) - 30 )
                and (cast ('2001-06-27' as date) + 30 ) 
 group by
    w_state,i_item_id
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Levy County','Marshall County','Terrell County','Terry County')
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('dark','blue','bisque','papaya','coral','lavender') and
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 22239822
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 12267527
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 76998840
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 132981190
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 111967817
            then (select avg(ss_ext_tax)
                  from store_sales
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (3,1,2,7,9,12) and
       d_year = 2001 and
       ca_state in ('KY','WI','MO'
                   ,'TX','TN','VA','IN')
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Dauphin County','Sierra County','Arthur County','Luce County',
                           'Terry County','Gogebic County','Saginaw County','Hubbard County')
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('IL', 'NY', 'MA')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NC', 'KS', 'VA')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('NJ', 'MT', 'MS')
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2) 
    and store.s_city in ('Hamilton','Macedonia','Sunnyside','Springfield','Woodland') 
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (1998) and
        ((i_category in ('Men','Women','Sports') and
          i_class in ('shirts','dresses','football')
         )
      or (i_category in ('Electronics','Children','Jewelry') and
          i_class in ('musical','infants','diamonds') 
        ))
...
select   
 ...
 from (
    (select 
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1999
 	and t_time between 53951 and 53951+28800 
