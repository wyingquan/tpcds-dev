...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2002-02-02','2002-08-04','2002-11-10')))
 ...
 from (
    (select 
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 13069 and 13069+28800 
 	and sm_carrier in ('ZHOU','DIAMOND')
     group by 
        w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Red Hill'
   ...
   and ib_lower_bound   >=  63649
   and ib_upper_bound   <=  63649 + 50000
...
and cc_county in ('Kittitas County','Pennington County','Daviess County','Ziebach County',
                  'Luce County'
)
and exists (select *
...
select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2001-03-18' as date) - 30 )
                and (cast ('2001-03-18' as date) + 30 ) 
 group by
    w_state,i_item_id
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('IN', 'OK', 'SD')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('VA', 'MO', 'OH')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('RI', 'NC', 'PA')
...
where 
	...
	and d_date between cast('2002-02-17' as date) 
				and (cast('2002-02-17' as date) + 30 )
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select  i_item_id
 ...
 and d_date between cast('2002-01-29' as date) and (cast('2002-01-29' as date) +  60 )
 and i_manufact_id in (126,787,153,919)
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (12,10,1,9,5,2) and
       d_year = 1999 and
       ca_state in ('PA','VA','IA'
                   ,'OR','NC','NY','WV')
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and store.s_city in ('Mount Pleasant','Oakwood')
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 19105384
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 23915831
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 111364173
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 33570695
            then (select avg(ss_ext_sales_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 43610372
            then (select avg(ss_ext_sales_price)
                  from store_sales
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2002-02-20' as date) and (cast('2002-02-20' as date) +  60 )
 and i_manufact_id in (938,757,858,851)
 and inv_quantity_on_hand between 100 and 500
 and cs_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('cyan','lime','brown','pale','orchid','lawn') and
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2) 
    and store.s_city in ('Liberty','Pleasant Grove','Oak Ridge','Red Hill','Shady Grove') 
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('1999-06-15' as date) - 30 )
                    and (cast ('1999-06-15' as date) + 30 )
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Wadena County','Hubbard County','Mobile County','Harmon County',
                           'Barrow County','Mesa County','Salem County','Franklin Parish')
...
select  
 ...
 where
  c.c_current_addr_sk = ca.ca_address_sk and
  ca_county in ('McCulloch County','Newport News city','Adams County','Grafton County','Livingston County') and
  cd_demo_sk = c.c_current_cdemo_sk and 
  exists (select *
          from store_sales,date_dim
          where c.c_customer_sk = ss_customer_sk and
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2)
    and store.s_county in ('Hubbard County','Richland County','Terry County','Contra Costa County')
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('turquoise','snow','magenta'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2000
 and     d_moy                   = 1
