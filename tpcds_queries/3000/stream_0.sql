...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast ('2001-06-05' as date) - 30 )
                    and (cast ('2001-06-05' as date) + 30 )
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('rose','salmon','moccasin','mint','plum','sienna') and
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Crossroads','Mount Pleasant','Fairview','Greenville','Lebanon') 
...
and cc_county in ('Franklin Parish','Wadena County','Walker County','Daviess County',
                  'San Miguel County'
)
and exists (select *
...
select  
 ...
 where
  ...
  ca_county in ('Rensselaer County','Carroll County','Lake County','Ripley County','Gilchrist County') and
...
select  i_item_id
 ...
 and d_date between cast('1999-05-18' as date) and (cast('1999-05-18' as date) +  60 )
 and i_manufact_id in (818,690,880,819)
     ...
     (select avg(ss_list_price) B2_LP
      ...
      where ss_quantity between 6 and 10
        and (ss_list_price between 188 and 188+10
          or ss_coupon_amt between 2522 and 2522+1000
     ...
     (select avg(ss_list_price) B3_LP
      ...
      where ss_quantity between 11 and 15
        and (ss_list_price between 43 and 43+10
          or ss_coupon_amt between 4529 and 4529+1000
     ...
     (select avg(ss_list_price) B4_LP
      ...
      where ss_quantity between 16 and 20
        and (ss_list_price between 179 and 179+10
          or ss_coupon_amt between 13125 and 13125+1000
     ...
     (select avg(ss_list_price) B5_LP
      ...
      where ss_quantity between 21 and 25
        and (ss_list_price between 71 and 71+10
          or ss_coupon_amt between 1570 and 1570+1000
     ...
     (select avg(ss_list_price) B6_LP
      ...
      where ss_quantity between 26 and 30
        and (ss_list_price between 4 and 4+10
          or ss_coupon_amt between 7830 and 7830+1000
...
select   
 ...
 from (
    (select 
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2001
 	and t_time between 18098 and 18098+28800 
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 11001111
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 19564716
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 128020557
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 39798948
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 19025945
            then (select avg(ss_ext_list_price)
                  from store_sales
...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2000) and
        ((i_category in ('Shoes','Sports','Children') and
          i_class in ('kids','fitness','newborn')
         )
      or (i_category in ('Music','Electronics','Jewelry') and
          i_class in ('country','stereo','consignment') 
        ))
...
select  i_item_desc 
 ...
 and d_date between cast('1998-04-20' as date) 
 				and (cast('1998-04-20' as date) + 30 )
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2)
    and store.s_county in ('Hubbard County','Orange County','Wadena County','San Miguel County',
                           'Maverick County','Terry County','Walker County','Franklin Parish')
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Adams County','Perry County','Richland County','Harmon County')
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Oak Hill'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  1728
   and ib_upper_bound   <=  1728 + 50000
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('papaya','indian','floral'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2002
 and     d_moy                   = 6
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast ('1999-04-22' as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1999-04-22' as date) - 30 )
                and (cast ('1999-04-22' as date) + 30 ) 
 group by
    w_state,i_item_id
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (2,7,8,1,9,12) and
       d_year = 2001 and
       ca_state in ('OK','AK','MN'
                   ,'GA','SD','TX','VA')
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('IN', 'NY', 'NM')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('RI', 'CO', 'MD')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('CT', 'IA', 'SD')
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and store.s_city in ('Mount Vernon','Union')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('2002-07-15','2002-08-18','2002-11-10')))
