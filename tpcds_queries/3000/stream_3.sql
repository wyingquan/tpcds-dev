...
where ss_item_sk = i_item_sk and
      ...
      d_year in (2001) and
        ((i_category in ('Electronics','Children','Sports') and
          i_class in ('audio','newborn','tennis')
         )
      or (i_category in ('Jewelry','Men','Books') and
          i_class in ('loose stones','sports-apparel','reference') 
        ))
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (4,8,10,12,1,6) and
       d_year = 1999 and
       ca_state in ('WI','IL','NH'
                   ,'ID','PA','CA','KY')
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('navy','green','medium'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2001
 and     d_moy                   = 1
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_country = 'United States'
  and ca_state in ('UT', 'GA', 'KY')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('OH', 'NC', 'MA')
...
select  
 ...
 where
  c.c_current_addr_sk = ca.ca_address_sk and
  ca_county in ('Carteret County','Bradley County','Madison County','Montgomery County','Carroll County') and
  cd_demo_sk = c.c_current_cdemo_sk and 
  exists (select *
          from store_sales,date_dim
          where c.c_customer_sk = ss_customer_sk and
...
select  i_item_id
 ...
 and i_manufact_id in (831,927,929,799)
...
select   
 ...
 from (
    (select 
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1998
 	and t_time between 48542 and 48542+28800 
...
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and inv_warehouse_sk   = w_warehouse_sk
     and inv_date_sk    = d_date_sk
     and d_date between (cast ('2002-02-01' as date) - 30 )
                    and (cast ('2002-02-01' as date) + 30 )
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and store.s_city in ('Newport','Enterprise','Crossroads','Riverview','Woodlawn') 
...
select  
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('2002-06-15' as date) - 30 )
                and (cast ('2002-06-15' as date) + 30 ) 
 group by
    w_state,i_item_id
...
and cc_county in ('Barrow County','Walker County','Williamson County','Raleigh County',
                  'Ziebach County'
)
and exists (select *
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and store.s_county in ('Arthur County','Luce County','Gogebic County','Fairfield County',
                           'Appanoose County','Anderson County','Jefferson Davis Parish','Terrell County')
...
select  c_last_name
 ...
 from (select ss_ticket_number
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
        ...
        and store.s_city in ('Woodland','Wilson')
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-03-19','1998-09-14','1998-11-19')))
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('green','dodger','peru','red','magenta','tan') and
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 69045263
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 100822287
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 15936992
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 34287019
            then (select avg(ss_ext_tax)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 58524608
            then (select avg(ss_ext_tax)
                  from store_sales
...
select c_last_name
   ...
   (select ss_ticket_number
    ...
    and date_dim.d_year in (2000,2000+1,2000+2)
    and store.s_county in ('Adams County','Sierra County','Maverick County','Gogebic County')
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2002-08-15' as date) 
                  and (cast('2002-08-15' as date) +  30 ) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
